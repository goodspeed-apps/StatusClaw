...
import { validateLogoUpload, MAX_UPLOAD_SIZE } from '@/lib/theme-config'
...
// POST /api/uploads/logo - Upload logo image (admin only)
async function handlePOST(req: AuthenticatedRequest, auth: AuthSuccess) {
  try {
    ...
    // Update branding config with new logo path
