...
import { spendTracker, getAggregatedSpendData, PRICING } from '@/lib/spend-tracker'
...
export async function GET() {
  try {
    ...
    // Get agent counts per model from actual spend records
    ...
    return NextResponse.json({ 
      models,
      usingRealData: !!aggregated.openRouterRealData || Object.keys(aggregated.byModel).length > 0,
...
function getAgentCountsByModel(): Record<string, number> {
  // Count unique agents per model from spend records (real data)
